 ...
 *
 */
public class MonthActiveStatService {
	public static void main(String[] args) {
		...
		//查询统计结果,入库
		Dataset<Row> df = spark.sql(ResourceUtil.loadResoureString("sql/stat_monthact.sql")) ;
//		df.show(10000,false) ;
			...
			public void call(Iterator<Row> t) throws Exception {
				...
				while(t.hasNext()){
					...
					if(!tntid.equals(curr_tntid)){
						//在mysql中准备该租户的统计表(日活)
						String create_ddl = String.format(ResourceUtil.loadResoureString("sql/create_monthact_ddl.sql"), tntid);
						conn.createStatement().execute(create_ddl);

						//先删除之前的删除
						String deleteSQL = String.format(ResourceUtil.loadResoureString("sql/delete_monthact.sql"),
								tntid ,row.getString(6));
						conn.createStatement().execute(create_ddl);

						//insert into ....
						String insertSQL = String.format(ResourceUtil.loadResoureString("sql/insert_monthact.sql"), tntid);
