 ...
 * 获得指定的月起始时刻(毫秒数)
 */
public class UmengMonthBeginUDF extends GenericUDF {
	...
	public ObjectInspector initialize(ObjectInspector[] arguments) throws UDFArgumentException {
		if(arguments.length != 1 && arguments.length != 2 && arguments.length !=3 ){
			throw new UDFArgumentException("参数不对!,函数调用形式为monthbegin(ms) , monthbegin(ms,-1) , monthbegin('2018-12' , 'yyyy-MM' , -1)");
		}
	...
	public Object evaluate(DeferredObject[] arguments) throws HiveException {
		...
		if(arguments.length == 1 || arguments.length == 2){
			...
			return DateUtil.monthBegin(new Date(ms) , offset) ;
			...
			return DateUtil.monthBegin(newDate , offset) ;
		}
	}

	public String getDisplayString(String[] children) {
		return "umeng_monthbegin" ;
	}
